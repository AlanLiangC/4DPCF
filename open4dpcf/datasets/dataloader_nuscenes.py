...
class NuscenesDataset(Dataset):
    def __init__(self, nusc, nusc_split, **kwargs) -> None:
    ...
    def __getitem__(self, idx):
        ...
        # save input points
        # for i in range(self.n_input):
        #     np.savetxt(f'ALTest/saved_points/{i}.txt', 
        #                input_points_tensor[input_tindex_tensor == i].numpy())
        # for i in range(self.n_output):
        #     np.savetxt(f'ALTest/saved_points/{i+self.n_input}.txt', 
...
def load_data(**kawrds):
    ...
    nusc = NuScenes(kawrds['nusc_version'], data_root)

    dataloader_train = create_loader(NuscenesDataset(nusc, nusc_split='train', **kawrds),
                                    shuffle=True,
                                    is_training=True,
                                    drop_last=True,
                                    **kawrds)
    dataloader_val = create_loader(NuscenesDataset(nusc, nusc_split='val', **kawrds),
                                    shuffle=False,
                                    is_training=False,
                                    drop_last=False,
                                    **kawrds)
